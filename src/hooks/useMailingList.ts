...
export default function useMailingList(
  ...
  const [mailingListSubmitting, setMailingListSubmitting] = React.useState(
    false
  )
  const [mailingListSubmitError, setMailingListSubmitError] = React.useState(
    false
  )
  const [
    mailingListSubmitSuccess,
    setMailingListSubmitSuccess,
  ] = React.useState(false)

  const [mailingListEmail, setMailingListEmail] = React.useState("")

  const handleMailingListSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    if (asyncEnabled) {
      e.preventDefault()
      if (!!mailingListEmail) {
