...
export interface IMDXPostListPageTemplateProps {
  pageContext: any
  data: any
}

export default function MDXPostListPageTemplate(
  props: IMDXPostListPageTemplateProps
) {
  // Data
  ...
  let posts: MdxContent[] = allMdx.nodes.map(node => new MdxContent(node)).filter((post: MdxContent) => !post.data.frontmatter.hidden)
  ...
  let postsHrefPrefix: string =
    categorySlugGlob === ALL_POSTS_GLOB
      ? `/${allPostsListSlug}`
      : `/${postCategoryListSlug}/${categorySlugGlob}`

  let previousPageLink: string | undefined = undefined
  if (currentPage > 1) {
    previousPageLink =
      postsHrefPrefix + "/" + (currentPage > 2 ? currentPage - 1 + "/" : "")
  }
  ...
  const contentTitle = renderTemplateTags(builtInPagesSettings.data.categoryPostListingPageSettings.contentTitle, templateTags)
  const seoTitle = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.seoTitle, templateTags)
  const seoDescription = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.seoDescription, templateTags)
  ...
  if (seoSettings.data.categoryPostListingPageSettings.seoImage.useSiteImage) {
    ...
    seoImageAlt = seoSettings.data.categoryPostListingPageSettings.seoImage.customSeoImageAlt !== "none" ? renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.seoImage.customSeoImageAlt, templateTags) : undefined
  }
  const ogTitle = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.openGraph.ogTitle, templateTags)
  const ogDescription = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.openGraph.ogDescription, templateTags)
  ...
  if (seoSettings.data.categoryPostListingPageSettings.openGraph.ogImage.ogUseCustomOgImage) {
    ...
    ogImageAlt = seoSettings.data.categoryPostListingPageSettings.openGraph.ogImage.ogCustomImageAlt !== "none" ? renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.openGraph.ogImage.ogCustomImageAlt, templateTags) : undefined
  }
  let twitterSiteUsername = seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardSiteUsername !== "none" ? renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardSiteUsername, templateTags) : undefined
  ...
  const twitterCardTitle = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardTitle, templateTags)
  const twitterCardDescription = renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardDescription, templateTags)
  ...
  if (seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardImage.twitterCardUseCustomImage) {
    twitterCardImageUrl = seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardImage.twitterCardCustomImage
    twitterCardImageAlt = seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardImage.twitterCardCustomImageAlt !== "none" ? renderTemplateTags(seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardImage.twitterCardCustomImageAlt, templateTags) : undefined
  }
  const twitterUseLargeImage: boolean = seoSettings.data.categoryPostListingPageSettings.twitterCards.twitterCardType === "summary-card-with-large-image"
