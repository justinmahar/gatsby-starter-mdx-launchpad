...
export default function PostCard(props: PostCardProps): JSX.Element {
  ...
  const postTitle: string = post.data.frontmatter.title ? post.data.frontmatter.title : 'Untitled Post';
    ...
    <Card key={post.data.id} className="mb-5 secondary">
      {hasFeaturedImage && (
        <Link to={postHref}>
          <Card.Img
            variant="top"
            src={post.data.frontmatter.featuredImage.featuredImageUrl}
            alt={
              post.data.frontmatter.featuredImage.featuredImageAlt !== 'none'
                ? post.data.frontmatter.featuredImage.featuredImageAlt
                : undefined
            }
          />
        </Link>
