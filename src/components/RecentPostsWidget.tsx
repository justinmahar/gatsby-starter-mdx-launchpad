...
export interface IRecentPostsWidgetProps { }

export default function RecentPostsWidget(props: IRecentPostsWidgetProps) {
  ...
  const posts: MdxContent[] = data.allMdx.nodes.map(node => new MdxContent(node)).filter((post: MdxContent) => !post.data.frontmatter.hidden)
